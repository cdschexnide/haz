...
import { Svg, Polygon, Rect, Text, TSpan } from "react-native-svg";
import {
  MaterialIcons,
  SimpleLineIcons,
  FontAwesome5,
...
import { SelectedTabIndexContext } from "../../../src/contexts/TabContext";

console.warn = () => {};

export interface InspectorWorkflowChevronIcon {
  type:
    | "MaterialIcons"
    | "FontAwesome5"
    | "SimpleLineIcons"
...
export function InspectorWorkflowChevron({
  icon,
  title,
  subtitle,
  width,
  ...
  const iconTop = isMultilineTitle ? 12 : 18;
  const titleY = isMultilineTitle ? 64 : 74;
  ...
  if (icon) {
    switch (icon.type) {
      ...
      case "MaterialCommunityIcons":
        ...
        if (icon.name === "file-check-outline") {
          IconComponent = (
            <MaterialCommunityIcons
              // name={icon.name}
        ...
        if (icon.name === "clipboard-list-outline") {
          IconComponent = (
            <MaterialCommunityIcons
              // name={icon.name}
        ...
        if (icon.name === "label-multiple-outline") {
          IconComponent = (
            <MaterialCommunityIcons
              // name={icon.name}
        ...
        if (icon.name === "compare-horizontal") {
          IconComponent = (
            <MaterialCommunityIcons
              // name={icon.name}
  ...
  const chevronContainerStyle = {
    height,
    transform: [{ scale: 1.1 }],
    opacity: disabled && title === "Excepted/\nLimited" ? 0.4 : 1,
  };
      ...
      >
        <View>
          <Svg style={styles.chevronStyle} height={height} width={width}>
            ...
            >
              {title?.split("\n").map((line, i) => (
                <TSpan x={width / 2} dy={i === 0 ? 0 : 14} key={i}>
                  {line}
                </TSpan>
              ))}
            </Text>
  ...
  return (
    <View key={chevKey} style={[{ height }, { transform: [{ scale: 1.1 }] }]}>
      <Svg style={styles.chevronStyle} height={height} width={width}>
        {IconComponent}
        <Rect
          width={width}
