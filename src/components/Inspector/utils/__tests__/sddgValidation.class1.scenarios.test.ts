...
function normalizeExpectedValue(value: string): string {
  ...
  if (/^(empty|none|n\/a)/i.test(value.trim())) return "";
...
function inferTargetKey(alteration: string): keyof ExtractedSDDGContent | null {
  ...
  if (/hazard class|division/i.test(alteration)) return "hazardClass";
...
function applyAlteration(
  ...
  if (key === "hazardClass") {
    ...
    updated.hazardClass = match ? match[0] : "1.4S";
    return updated;
  }
...
describe("SDDG recommended frustrations - Class 1 scenarios", () => {
  test("scenario docs align with hazardousMaterialsList", () => {
    ...
    for (const scenario of class1ScenarioFixtures) {
      ...
      if (
        ...
        expectedSubsidiaryRisk !== material.subsidiaryRisk
      ) {
        mismatches.push(
