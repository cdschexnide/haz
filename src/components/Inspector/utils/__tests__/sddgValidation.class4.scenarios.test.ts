...
function shouldSkipAlteration(
  ...
  const combined =
    `${alteration.alteration} ${alteration.tests || ""} ${alteration.category || ""} ${alteration.expectedResult || ""}`.toLowerCase();
  if (
...
function applyAlteration(
  ...
  if (key === "hazardClass") {
    ...
    updated.hazardClass = match ? match[0] : "4.1";
    return updated;
  }
...
describe("SDDG recommended frustrations - Class 4 scenarios", () => {
