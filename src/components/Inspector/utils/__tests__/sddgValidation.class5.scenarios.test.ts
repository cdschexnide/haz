...
import { getInhalationHazardRequirement } from "@/utils/specialProvisions";
...
function normalizeExpectedValue(value: string): string {
  ...
  if (/(^|\b)(empty|none|n\/a)(\b|$)/i.test(cleaned)) return "";
...
function buildExtractedContent(
  ...
  return {
    ...
    properShippingName:
      material?.properShippingName?.trim() || expectedPsn || "",
    hazardClass:
...
function inferTargetKey(alteration: string): keyof ExtractedSDDGContent | null {
  ...
  if (/hazard class|division|oxidizer/i.test(alteration)) return "hazardClass";
...
function applyAlteration(
  ...
  if (key === "hazardClass") {
    ...
    updated.hazardClass = match ? match[0] : "5.1";
    return updated;
  }
  if (key === "subsidiaryRisk") {
    ...
    updated.subsidiaryRisk = match ? match[0] : "8";
    return updated;
  }
  if (key === "packingGroup") {
    if (/empty/i.test(alteration)) {
...
describe("SDDG recommended frustrations - Class 5 scenarios", () => {
  test("scenario docs align with hazardousMaterialsList", () => {
    ...
    for (const scenario of class5ScenarioFixtures) {
      ...
      if (
        ...
        expectedPackingGroup !== material.packingGroup
      ) {
        mismatches.push(
  ...
  test("correct SDDG data does not trigger recommendations for keys 7,10,12-15,17", () => {
    ...
    for (const scenario of class5ScenarioFixtures) {
      ...
      const inhalationRequirement = getInhalationHazardRequirement(
      ...
      const relevantFlags = inhalationRequirement.requiresInhalationHazard
        ? flagged.filter(key => key !== "properShippingName")
        : flagged;

      if (relevantFlags.length > 0) {
        failures.push(
          `Scenario ${scenario.scenario}: unexpected recommendations for ${relevantFlags.join(", ")}`
        );
      }
    }
