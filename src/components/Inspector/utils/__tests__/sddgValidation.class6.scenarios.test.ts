...
function normalizeExpectedValue(value: string): string {
  ...
  if (/^(empty|none|n\/a|\(empty\))/i.test(value.trim())) return "";
...
function buildExtractedContent(
  scenario: (typeof class6ScenarioFixtures)[number]
  ...
  const aircraftType =
    getExpectedValue(expected, "Key 7") ||
  ...
  return {
    ...
    packingGroup:
      getExpectedValue(expected, "Key 15") || material?.packingGroup || "",
    quantityAndPacking: "",
    packingInstruction:
...
function inferTargetKey(alteration: string): keyof ExtractedSDDGContent | null {
  if (/technical name/i.test(alteration)) return null;
  ...
  if (/UN number|UN\d{4}|NA\d{4}/i.test(alteration)) return "unIdNo";
  ...
  if (/hazard class|division|toxic|infectious/i.test(alteration)) {
    return "hazardClass";
  }
...
function applyAlteration(
  ...
  if (key === "hazardClass") {
    ...
    updated.hazardClass = match ? match[0] : "6.1";
    return updated;
  }
...
describe("SDDG recommended frustrations - Class 6 scenarios", () => {
    ...
    for (const scenario of class6ScenarioFixtures) {
      ...
      if (
        ...
        !normalizeParagraph(material.packagingParagraph || "")
          ...
          .includes(normalizeParagraph(expectedPackingInstruction))
      ) {
        mismatches.push(
