...
function inferTargetKey(alteration: string): keyof ExtractedSDDGContent | null {
  ...
  if (/hazard class|division|corrosive/i.test(alteration)) return "hazardClass";
...
function applyAlteration(
  ...
  if (key === "hazardClass") {
    ...
    updated.hazardClass = match ? match[0] : "8";
    return updated;
  }
  if (key === "subsidiaryRisk") {
    ...
    const match = alteration.match(/\b\d\b/);
    updated.subsidiaryRisk = match ? match[0] : "3";
    return updated;
  }
  if (key === "packingGroup") {
...
describe("SDDG recommended frustrations - Class 8 scenarios", () => {
