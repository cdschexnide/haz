...
function inferTargetKey(alteration: string): keyof ExtractedSDDGContent | null {
  ...
  if (/UN number|UN\d{4}|ID\d{4}/i.test(alteration)) return "unIdNo";
  ...
  if (/hazard class|division|class 9/i.test(alteration)) return "hazardClass";
...
function applyAlteration(
  ...
  if (key === "unIdNo") {
    const match = alteration.match(/\b[A-Z]{2}\d{4}|ID\d{4}\b/);
    if (match) {
  ...
  if (key === "hazardClass") {
    const match = alteration.match(/\b\d\b/);
    updated.hazardClass = match ? match[0] : "9";
    return updated;
  }
...
describe("SDDG recommended frustrations - Class 9 scenarios", () => {
