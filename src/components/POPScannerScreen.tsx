/**
 * POPScannerScreen - Camera screen for scanning POP markings using OCR
 *
...
const POPScannerScreen: React.FC<POPScannerScreenProps> = ({ navigation }) => {
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      const { fields, confidence, issues, detectedType } = extractPOPMarkingFromText(ocrText);
      console.log('Extraction complete:', { fields, confidence, issues, detectedType });

      // Always update store with extracted values (even if partially successful)
      if (store.hazProPreparerContext.packaging) {
        ...
        if (fields) {
          console.log('Updating store with parsed fields...');
          store.hazProPreparerContext.packaging.inputPOPMarking.A = fields.A;
          store.hazProPreparerContext.packaging.inputPOPMarking.B = fields.B;
          store.hazProPreparerContext.packaging.inputPOPMarking.C = fields.C;
          store.hazProPreparerContext.packaging.inputPOPMarking.D = fields.D;
          store.hazProPreparerContext.packaging.inputPOPMarking.E = fields.E;
          store.hazProPreparerContext.packaging.inputPOPMarking.F = fields.F;
          store.hazProPreparerContext.packaging.inputPOPMarking.G = fields.G;
          store.hazProPreparerContext.packaging.inputPOPMarking.H = fields.H;
          console.log('Store updated with fields:', store.hazProPreparerContext.packaging.inputPOPMarking);
        } else {
          console.log('⚠️ No fields extracted - store not updated');
        }
      }

      // Navigate to results screen for review
      console.log('Navigating to POPScanResultsScreen...');
      navigation.navigate('POPScanResultsScreen', {
        imageUri: photo.uri,
        rawOCRText: ocrText,
        parsedFields: fields || {
          type: 'unknown',
...
export default POPScannerScreen;
