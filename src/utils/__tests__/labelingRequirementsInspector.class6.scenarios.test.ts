...
describe("Labeling requirements - Class 6 scenarios", () => {
  test("expected labels are present in evaluateLabelingRequirements output", () => {
    ...
    for (const scenario of class6ScenarioFixtures) {
      ...
      for (const label of scenario.expectedPackage.labels) {
        ...
        if (/infectious|biological/i.test(label)) {
          const primary = result["Primary Hazard"] || [];
          const expected = "Class 6.2";
        ...
        if (/toxic/i.test(label)) {
          const hazardClass = extractHazardClassFromLabel(label) || "6.1";
          const primary = result["Primary Hazard"] || [];
        ...
        if (/inhalation hazard/i.test(label)) {
          const hasInhalation = Object.keys(result).some(key =>
            key.toUpperCase().includes("INHALATION HAZARD")
          );
          if (!hasInhalation) {
            failures.push(
              `Scenario ${scenario.scenario}: missing TOXIC INHALATION HAZARD label requirement`
            );
          }
        }

        if (/corrosive|flammable|subsidiary/i.test(label)) {
