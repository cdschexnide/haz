...
function findInvalidPackagingCode(alterations: { alteration: string }[]): string | null {
  for (const alteration of alterations) {
    if (!/POP marking|packaging code/i.test(alteration.alteration)) continue;
...
describe("POP marking validation - Class 9 scenarios", () => {
  test("allowed packaging codes are valid for their packaging paragraph when POP is required", () => {
    ...
    for (const scenario of class9ScenarioFixtures) {
      ...
      if (!scenario.expectedPackage.pop.required) continue;
  ...
  test("known invalid packaging codes fail validation when POP is required", () => {
    const failures: string[] = [];
  ...
  test("packing group determines valid POP code letters when POP is required", () => {
    const failures: string[] = [];
